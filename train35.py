...
weight=[1,1]
def binarySegmentationLosses( logit, target):
    logit = torch.sigmoid(logit)
    logit = logit.view(-1)
    target = target.view(-1)
    loss = -torch.mean(
        weight[1] * target * torch.log(logit + 1e-7) + weight[0] * (1 - target) * torch.log(
            1 - logit + 1e-7))
    return loss
...
def dice_loss(prediction, target):
    ...
    i_flat = prediction.view(N, -1)
    t_flat = target.view(N, -1)
...
# train_data = DataLoader(my_val, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=0, drop_last=True)
# val_data = DataLoader(my_train, batch_size=config.BATCH_SIZE, shuffle=False, num_workers=0,drop_last=True)
...
for epoch in range(1, config.EPOCH_NUMBER + 1):
    ...
    for i, sample in enumerate(train_data):
        ...
        img_label = Variable(sample["label"].to(device)) #torch.Size([4, 256, 256])

        # 训练
        out_ = net(img_data) #([4, 2, 256, 256])
        out = F.log_softmax(out_, dim=1)

        # print(out.shape,img_label.shape,out_.shape) #[4, 2, 256, 256],4, 256, 256,[4, 2, 256, 256]

        # loss = criterion(out, img_label.long())
        # loss = binarySegmentationLosses(out[:,0,:,:], img_label.long())
        # loss = dice_loss(out_[:, 0, :, :], img_label.long())+ dice_loss(out_[:, 1, :, :], 1.0 -img_label.long())
        # loss = 0.5 * (dice_loss(out_[:, 0, :, :], img_label.long()) + dice_loss(out_[:, 1, :, :],
        #                                                                         1.0 - img_label.long())) + criterion(out, img_label.long())
        out2 = torch.sigmoid(out_[:, 0, :, :])
        loss =  dice_loss(out2, img_label.long()) + criterion(out, img_label.long())
    ...
    for j, sample in enumerate(val_data):
        ...
        # print(valImg.shape, valLabel.shape)
        ...
        # loss = 0.5 * (dice_loss(out_[:, 0, :, :], img_label.long()) + dice_loss(out_[:, 1, :, :],
        #                                                                         1.0 - img_label.long())) + criterion(
        #     out, img_label.long())

        # loss = 0.5 * (dice_loss(out[:, 0, :, :], img_label.long()) + dice_loss(out[:, 1, :, :],
        #                                                                        1.0 - img_label.long()))
        out2 = torch.sigmoid(out_[:, 0, :, :])
        # print(out2.shape,img_label.shape)
        loss = dice_loss(out2, valLabel.long()) + criterion(out, valLabel.long())
